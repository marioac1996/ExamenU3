...
export class Example2Service {
	...
	private exampleCollection: AngularFirestoreCollection<Example2>;


  constructor(private afs: AngularFirestore) {
  	this.exampleCollection =this.afs.collection<Example2>('examples');
   ...
   getExamples(): Observable<Example2[]>{
  		return this.examples;
  	}

  	getExample(id:string):Observable<Example2>{
  		return this.exampleCollection.doc<Example2>(id).valueChanges().pipe(
  			take(1),
  			map(example=>{
  				example.id = id;
  				return example;
    ...
    addExample(example:Example2):Promise<DocumentReference>{
      return this.exampleCollection.add(example);
    }
    updateExample(example:Example2):Promise<void>{
